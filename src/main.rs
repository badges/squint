...
lazy_static! {
    static ref SVG_BASE_URL: &'static str =
        option_env!("SVG_BASE_URL").unwrap_or(DEFAULT_SVG_BASE_URL);
}
...
async fn main() -> Result<(), GenericServerError> {
    ...
    let port =
        option_env!("PORT").map_or(DEFAULT_PORT, |p| p.parse::<u16>().unwrap_or(DEFAULT_PORT));
